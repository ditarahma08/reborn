...
const Index = (props) => {
  const { page, categoryServices, brandName, topServices, pageContent, searchProducts } = props;
  ...
  const breadcrumbList = [
    { id: 1, name: 'Beranda', path: '/servis' },
    {
      ...
      path: `/servis/${categoryServices}`
    },
    {
      id: 3,
      name: Helper.slugToTitle(brandName),
      path: ''
    }
  ];
  ...
  const { data, mutate, isValidating } = useSWR(
    `${
      process.env.API_URL
    }v2/search/products/${categoryServices}/brand-product/${brandName}?${Helper.objectToQueryString(
      paramsSwr
    )}`,
    Helper.fetcher,
    optionsSwr
  ...
  const handleClickProduct = (item, car) => {
    ...
    if (!isUndefined(car) && categoryServices !== 'oli') {
      const params = {};
      goToWsList(params);
    }
    ...
    if (!isUndefined(car) && !item?.compatibility && categoryServices === 'oli') {
      setOpenCheck(true);
    }
  };
...
export async function getServerSideProps({ query }) {
  const { categoryServices, brandName, page } = query;
  ...
  let apiSearchProducts = `${
    process.env.API_URL
  }v2/search/products/${categoryServices}/brand-product/${brandName}?${JSON.stringify(params)}`;

  const [pageContentRes, topServicesRes, searchProductsRes] = await Promise.all([
    fetch(
      `${process.env.API_URL}v2/search/products/${categoryServices}/brand-product/${brandName}?initial=true`
    ),
